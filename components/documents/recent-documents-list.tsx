      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {documents.length === 0 ? (
          ...
          documents.map((doc) => {
            ...
            const wrapperProps = isCompleted ? { href: `/documents/${doc.id}` } : {};
            const formattedDate = new Date(doc.uploadedDate).toLocaleDateString(undefined, { 
              year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' 
            });

            return (
              <Wrapper key={doc.id} {...wrapperProps} className="block">
                <div className={`flex items-start gap-4 p-4 rounded-lg border transition-colors ${
                  isCompleted ? 'border-transparent hover:border-border hover:bg-muted/30' : 'border-border/50 bg-muted/10 opacity-90'
                }`}>
                   ...
                   <div className="flex-1 min-w-0">
                     ...
                     <p className="text-xs text-muted-foreground mb-3">{doc.source} • Uploaded {formattedDate}</p>
                     ...
                     {isCompleted && (
                       <div className="flex justify-between items-center text-xs text-muted-foreground pt-1.5 border-t border-border/50">
                         <span>{doc.extractedTransactions || 0} extracted • {doc.selectedTransactions || 0} selected</span>
