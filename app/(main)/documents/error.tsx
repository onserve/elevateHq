...
export default function DocumentsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[Documents Error]', error); }, [error]);
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      ...
      <h2 className="text-2xl font-bold text-foreground mb-2">Failed to load documents</h2>
      <p className="text-muted-foreground max-w-md mb-8 text-sm">{error.message || "We couldn't load your documents. Please try again."}</p>
